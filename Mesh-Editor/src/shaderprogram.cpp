...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifJointTransforms(-1), unifBindMatrices(-1), attrWeights(-1), attrJointIndices(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJointIndices = context->glGetAttribLocation(prog, "vs_JointIndices");
...
void ShaderProgram::setRenderMode(int renderMode)
{
    useMe();
    if (unifRenderMode != -1)
        context->glUniform1i(unifRenderMode, renderMode);
}

void ShaderProgram::setBindMatrices(std::vector<glm::mat4> &m)
{
    if (!m.size()) return;
    useMe();
    if (unifBindMatrices != -1)
    {
        context->glUniformMatrix4fv(unifBindMatrices, m.size(), false, &m[0][0][0]);
        //print(m[10]);
    }
}

void ShaderProgram::setJointTransforms(std::vector<glm::mat4> &m)
{
    if (!m.size()) return;
    useMe();
    if (unifJointTransforms != -1)
    {
        context->glUniformMatrix4fv(unifJointTransforms, m.size(), false, &m[0][0][0]);
        //print(m[10]);
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrWeights != -1 && d.bindWeights()) {
        ...
        context->glVertexAttribPointer(attrWeights, INFLUENCE_NUM_JOINT, GL_FLOAT, false, 0, NULL);
    }
    if (attrJointIndices != -1 && d.bindJointIndices()) {
        context->glEnableVertexAttribArray(attrJointIndices);
        context->glVertexAttribIPointer(attrJointIndices, INFLUENCE_NUM_JOINT, GL_INT,  0, NULL);
    }
    ...
    if (attrJointIndices != -1) context->glDisableVertexAttribArray(attrJointIndices);
